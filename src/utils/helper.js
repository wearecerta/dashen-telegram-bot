...
export async function safeSend(ctx, text, keyboard = null, options = {}) {
  try {
    if (keyboard) {
      await ctx.reply(text, { ...options, parse_mode: "Markdown", ...keyboard });
    } else {
    ...
    if (error.message.includes("can't parse entities")) {
      ...
      if (keyboard) {
        await ctx.reply(text.replace(/[*_`]/g, ''), { ...options, parse_mode: undefined, ...keyboard });
      } else {
        await ctx.reply(text.replace(/[*_`]/g, ''), { ...options, parse_mode: undefined });
      }
    } else {
      throw error;
    }
