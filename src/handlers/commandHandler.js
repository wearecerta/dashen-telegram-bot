...
export async function handleEvent(ctx) {
  ...
  if (!title || !date) {
    return ctx.reply("Usage: /event Party 2026-04-12\n(Event name and date required)");
  }
...
export async function handleStatus(ctx) {
  try {
    ...
    if (!currentEventId) {
      return ctx.reply("❌ No active event found in this group. Create one by saying 'let's ...' or use /event");
    }
...
export async function handleCancel(ctx) {
  try {
    ...
    const finalMsg = randomMsg.replace(/{event}/g, event.title).replace(/{name}/g, ctx.from.first_name);
